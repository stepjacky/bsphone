...
class Sina {
    ...
    private $appId = "1047254860";
    private $secuKey="8a53b23ae44e7ef1a8c7f26b81fbd3bc";
    private $callback="http://www.bsphone.com/sinalogin/login";

    public function sinaAuthUrl($state){
        $o = new SaeTOAuthV2( $this->appId , $this->secuKey );
        $code_url = $o->getAuthorizeURL($this->callback.'?lstate='.$state);
        return $code_url;
    }

    public function getAuthoriedToken($code){
        $o = new SaeTOAuthV2( $this->appId , $this->secuKey );
        $CI =& get_instance();
        $CI->load->library('nsession');
        $lstate = $CI->nsession->userdata('lstate');
        $keys = array();
        $keys['code'] = $code;
        $keys['redirect_uri'] = $this->callback.'?lstate='.$lstate;
        $token = null;
        try {
    ...
    public function getLoginUserId($code){
        $token = $this->getAuthoriedToken($code);


        $c   = new SaeTClientV2(  $this->appId , $this->secuKey , $token['access_token'] );
        $uid_get = $c->get_uid();
        $uid = $uid_get['uid'];
