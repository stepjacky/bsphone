...
abstract class OAuth2_Provider
{
	...
	*/	
	public function authorize($options = array())
	{
		...
		get_instance()->nsession->set_userdata('state', $state);
	...
	*/	
	public function access($code, $options = array())
	{
        ...
        //get_instance()->nsession->userdata('state')
        if (isset($_GET[$this->state_key]) AND $_GET[$this->state_key] != $_SESSION['state'] )
        {
            $state    = $_GET[$this->state_key];
            $state_s  = $_SESSION['state'];

            show_error($state."--".$state_s);
