<?php if ( ! defined('BASEPATH')) exit('No direct script access allowed');

$config['oauth2']['weibo'] = array(
	'name' => '新浪微博',
	'id' => '1047254860',
	'secret' => '8a53b23ae44e7ef1a8c7f26b81fbd3bc',
        'extra' => ''
);

$config['oauth2']['qq'] = array(
	'name' => 'QQ',
        'id' => '100339887',
        'secret' => 'df45b4314e6fffccbff01a22f284dceb',
        'extra' => ''
);

$config['oauth2']['tweibo'] = array(
	'name' => '腾讯微博',
        'id' => '801312217',
        'secret' => 'e2a3c79826953b70ebab48784bcdd6c2',
        'extra' => ''
);

$config['oauth2']['baidu'] = array(
	'name' => '百度',
        'id' => 'ptHojgnRNf8vXnH4koFFMbcy',
        'secret' => 'kvPBt1z9wIBM4ffu2n3Nim2GBVMT6igW',
        'extra' => ''
);

$config['oauth2']['360'] = array(
	'name' => '360',
        'id' => '9f27bc91bbe042f6750cf4a5171190b1',
        'secret' => '3afe24e21595de9ef0ae4c626d48e058',
        'extra' => ''
);

$config['oauth2']['163'] = array(
	'name' => '网易微博',
        'id' => 'yKaDegptKluarca0',
        'secret' => '1ktimIzcJVUhEbB8Ss7OI0iJwj2imoIr',
        'extra' => ''
);

$config['oauth2']['sohu'] = array(
	'name' => '搜狐微博',
        'id' => 'npXD5HCSYrCPdLk53QNq',
        'secret' => '5j5Ttq8Nq)Uspp7rr!lyFOpISqE$6JSP%f9BX04x',
        'extra' => ''
);

$config['oauth2']['douban'] = array(
	'name' => '豆瓣',
        'id' => '06807208f9e50c451381561aedaf24cf',
        'secret' => 'f85e05ce208e75ab',
        'extra' => ''
);

$config['oauth2']['taobao'] = array(
	'name' => '淘宝',
        'id' => '21384541',
        'secret' => '4d273b331d74168564d8fc9ded45e058',
        'extra' => ''
);

$config['oauth2']['tianyi'] = array(
	'name' => '天翼',
        'id' => 'dummy_id',
        'secret' => 'dummy_secret',
        'extra' => '未申请到应用,如果你可以协助申请，请联系234267695@qq.com.'
);

$config['oauth2']['renren'] = array(
	'name' => '人人',
        'id' => '86d5c22ab0594f1ab5a86ac9b1f888be',
        'secret' => '149ff08caa564503b0f9f12b7eb7a86b',
        'extra' => '感谢longyuan妹子帮助申请应用'
);

$config['oauth2']['10086'] = array(
	'name' => '移动微博',
        'id' => '0ed01c59e5e229ea1788e75199a89283',
        'secret' => '16e29aab31bf9a55aaa5022ca173d187',
        'extra' => '莫名抽风，说域名未验证，未发现在什么地方可以验证.'
);

$config['oauth2']['feixin'] = array(
        'name' => '飞信',
        'id' => 'nokey',
        'secret' => 'nosecret',
        'extra' => '未申请到应用,如果你可以协助申请，请联系234267695@qq.com.'
);

$config['oauth2']['kaixin'] = array(
	'name' => '开心网',
        'id' => 'nokey',
        'secret' => 'nosecret',
        'extra' => '未申请到应用,如果你可以协助申请，请联系234267695@qq.com.'
);

$config['oauth2']['duoshuo'] = array(
	'name' => '多说',
        'id' => 'dummy_id',
        'secret' => 'dymmy_secret',
        'extra' => '<h2>使用多说登录</h2><div class="ds-login"></div>'
);

/* End of file oauth2.php */
/* Location: ./application/config/oauth2.php */